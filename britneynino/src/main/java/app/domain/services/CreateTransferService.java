...
public class CreateTransferService {
	...
	public void createTransfer(Transfer transfer, SystemRole creatorRole) throws BusinessException {
		...
		BankAccount source = bankAccountPort.findByAccountNumber(transfer.getSourceAccount().getAccountNumber());
		if (source == null) {
		...
		BankAccount destination = bankAccountPort
		...
		if (transfer.requiresApproval()) {
			...
			OperationLog log = new OperationLog(OperationType.TRANSFER_PENDING_APPROVAL, transfer.getCreatorUserId(),
					creatorRole, transfer.getTransferId());
			...
			OperationLog log = new OperationLog(OperationType.TRANSFER_EXECUTED, transfer.getCreatorUserId(),
					creatorRole, transfer.getTransferId());
			log.addDetail("amount", transfer.getAmount());
			log.addDetail("sourceAccount", source.getAccountNumber());
