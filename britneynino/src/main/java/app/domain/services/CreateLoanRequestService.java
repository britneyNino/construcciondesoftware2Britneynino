...
public class CreateLoanRequestService {
	...
	public void requestLoan(Loan loan, String creatorUserId, SystemRole creatorRole) throws BusinessException {
		...
		Client client = clientPort.findByIdentification(loan.getClient().getIdentification());
		if (client == null) {
		...
		OperationLog log = new OperationLog(OperationType.LOAN_REQUESTED, creatorUserId, creatorRole,
				loan.getProductId());
